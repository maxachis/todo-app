...
#### Scenario: Detail sidebar shows Focus button
- **WHEN** a node's details are displayed in the graph sidebar
- **THEN** a "Focus" button SHALL be present allowing the user to enter ego/neighborhood focus mode for that node

#### Scenario: Filters card shows cluster toggle
- **WHEN** the user views the graph Filters card
- **THEN** a "Show org clusters" checkbox SHALL be present, defaulting to checked
