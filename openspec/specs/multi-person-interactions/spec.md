...
The interaction API create endpoint SHALL accept a `person_ids` field (list of integers, at least one required). The update endpoint SHALL accept an optional `person_ids` field (when provided, replaces the full set). The response schema SHALL include a `person_ids` field (list of integers) instead of the previous `person_id` field.
...
#### Scenario: Create interaction via API with one person
- **WHEN** a client sends POST to `/api/interactions/` with `person_ids: [1]`, type, date, and notes
- **THEN** the server creates the interaction with one person and responds with status 201 including `person_ids: [1]`
...
#### Scenario: Update interaction without changing people
- **WHEN** a client sends PUT to `/api/interactions/{id}/` without a `person_ids` field
- **THEN** the interaction's people set remains unchanged

#### Scenario: Interaction response includes person_ids
- **WHEN** a client sends GET to `/api/interactions/` or `/api/interactions/{id}/`
- **THEN** each interaction object includes a `person_ids` field containing the list of associated person IDs
